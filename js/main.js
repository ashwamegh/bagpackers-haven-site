...
(function ($) {
	...
	$(".checkin_date, .checkout_date").datepicker({
		format: "m/d/yyyy",
		autoclose: true,
		maxDate: new Date(2021, 1 - 1, 1),
	});

	$("#enquire-now-form").submit((e) => {
		e.preventDefault();
		const $inputs = $("#enquire-now-form :input");
		...
		let { checkin_date, checkout_date, tent_package } = values;
		...
		checkin_date =
			...
			"-" +
			checkin_date.toString().substr(4, 3) +
			"-" +
			checkin_date.getFullYear();
		checkout_date =
			checkout_date.getDate() +
			"-" +
			checkout_date.toString().substr(4, 3) +
			"-" +
